...
app.layout = html.Div(style={
    ...
    html.Div(style={"maxWidth": "1800px", "margin": "0 auto", "padding": "40px 36px", "position": "relative"}, children=[
        ...
        # Network Link Analysis Section - Minimal Professional Design
        html.Div(style={
            ...
            html.Div(style={
                ...
                html.H2("Network Link Analysis", style={
                    "margin": "0",
                    "fontSize": "32px",
                    "fontWeight": "600",
                    "letterSpacing": "-0.5px",
            ...
            # Key Indicators Grid - Enhanced Styling
            html.Div(style={
                "marginBottom": "36px"
            }, children=[
                html.Div(style={
        ...
        html.Div(style={
            ...
            "gap": "28px",
                    ...
                    html.Div(id="link-state-indicator", style={
                        **CARD_STYLE,
                        "padding": "36px 32px",
                        "textAlign": "center",
                        "border": f"1px solid {THEME_COLORS['border']}",
                    ...
                    html.Div(id="mac-retransmission-indicator", style={
                        **CARD_STYLE,
                        "padding": "36px 32px",
                        "textAlign": "center",
                        "border": f"1px solid {THEME_COLORS['border']}",
            ...
            # Analysis Panels Grid - Enhanced Styling
            html.Div(style={
                "marginTop": "8px"
            }, children=[
                html.Div(style={
        ...
        html.Div(style={
            "display": "grid",
                    ...
                    html.Div(id="link-state-explanation", style={
                        **CARD_STYLE,
                        "padding": "36px 32px",
                        "border": f"1px solid {THEME_COLORS['border']}",
                        "boxShadow": "0 2px 8px rgba(0, 0, 0, 0.15), 0 1px 4px rgba(0, 0, 0, 0.1)",
        ...
        # Charts Section - Enhanced Grid Layout
        html.Div(className="chart-grid-3", style={
            ...
            html.Div(
                className="card-hover-effect",
                style={**GRAPH_STYLE, "cursor": "help"},
                title="RTT Variance (Jitter) Over Time: Shows RTT variance/jitter over time. Mean RTT alone hides instability; variance reveals network turbulence.",
                children=[
            ...
            html.Div(
                className="card-hover-effect",
                style={**GRAPH_STYLE, "cursor": "help"},
                title="RSSI vs RTT: Relationship between signal strength and network latency. Color-coded by link state. Shows that strong signal does not always imply low latency.",
                children=[
        ...
        # RSSI Variation Chart - Full Width (Larger)
        html.Div(style={
            "marginBottom": "48px"
        }, children=[
            html.Div(
                className="card-hover-effect",
                style={**GRAPH_STYLE, "cursor": "help"},
                title="RSSI Variation Relative to Rolling Mean: Shows difference between current RSSI and rolling average (ΔRSSI). Captures signal dynamics, mobility, and fading effects.",
                children=[
...
@app.callback(
    ...
    Output("link-state-indicator", "children"),
    Output("link-state-explanation", "children"),
    Output("cross-layer-insight", "children"),
    Output("mac-retransmission-indicator", "children"),
...
def update_dashboard_components(n: int):
    ...
    if df.empty:
        empty_layout = create_empty_figure_layout()
        return conn_badges, [], [], html.Div("Collecting data..."), html.Div("Collecting data..."), html.Div("Collecting data..."), html.Div("Collecting data..."), empty_layout, empty_layout, empty_layout, empty_layout, empty_layout, empty_layout, empty_layout

    try:
        latest_metrics = df.iloc[-1]
    except (IndexError, KeyError):
        empty_layout = create_empty_figure_layout()
        return conn_badges, [], [], html.Div("No data available"), html.Div("No data available"), html.Div("No data available"), html.Div("No data available"), empty_layout, empty_layout, empty_layout, empty_layout, empty_layout, empty_layout, empty_layout
    ...
    # Generate optimization components with professional styling
    try:
        link_state, state_color = calculate_link_state(df)
        link_state_indicator = html.Div([
        ...
        link_state_indicator = html.Div("Error generating indicator")
        link_state_explanation = html.Div("Error generating explanation")
        cross_layer_insight = html.Div("Error generating insight")
        mac_retransmission = html.Div("Error")
    ...
    try:
        ...
        empty_layout = create_empty_figure_layout()
        chart1 = chart3 = chart4 = chart5 = chart6 = chart7 = chart8 = empty_layout
    
    return conn_badges, kpi_cards, metric_cards, link_state_indicator, link_state_explanation, cross_layer_insight, mac_retransmission, chart1, chart3, chart4, chart5, chart6, chart7, chart8

# ================= OPTIMIZATION COMPONENTS =================

def generate_link_state_explanation(df: pd.DataFrame, latest: pd.Series) -> list:
    """
    Generates link state explanation panel with detailed metrics breakdown.
    ...
    # Determine state
    state, state_color = calculate_link_state(df)
    
    signal_str = f"{signal_mean:.0f}%" if signal_mean is not None else "N/A"
    rtt_str = f"{rtt_mean:.1f}ms" if rtt_mean is not None else "N/A"
    
    if state == "STABLE":
        explanation = f"Link is stable with consistent performance. Signal: {signal_str} ({signal_trend}), RTT: {rtt_str} ({rtt_trend}), PHY rates {phy_trend}."
    ...
    return html.Div([
        ...
        html.Div(style={
            ...
            html.Div(style={
                ...
                html.Span("Current State: ", style={"fontWeight": "600", "color": THEME_COLORS["text_secondary"], "fontSize": "14px"}),
        ...
        html.Div(style={
            ...
            "gap": "20px",
            "paddingTop": "24px",
            "borderTop": f"2px solid {THEME_COLORS['border']}"
        }, children=[
            html.Div(style={"textAlign": "center"}, children=[
                html.Div("RSSI Trend", style={
                    "fontSize": "10px",
                ...
                html.Div(signal_trend.upper(), style={
                    "fontSize": "16px",
                    "color": THEME_COLORS["text_primary"],
                    "fontWeight": "700",
                    "letterSpacing": "0.5px"
            ...
            html.Div(style={"textAlign": "center"}, children=[
                ...
                html.Div(rtt_trend.upper(), style={
                    "fontSize": "16px",
                    "color": THEME_COLORS["text_primary"],
                    "fontWeight": "700",
                    "letterSpacing": "0.5px"
            ...
            html.Div(style={"textAlign": "center"}, children=[
                ...
                html.Div(phy_trend.upper(), style={
                    "fontSize": "16px",
                    "color": THEME_COLORS["text_primary"],
                    "fontWeight": "700",
                    "letterSpacing": "0.5px"
...
def generate_cross_layer_insight(df: pd.DataFrame, latest: pd.Series) -> list:
    """
    Generates cross-layer insight explaining PHY-Network layer interactions.
    ...
    Returns:
        HTML elements with cross-layer insight
    """
    if df.empty or len(df) < 10:
        return html.Div("Insufficient data for cross-layer analysis.", style={
            "color": THEME_COLORS["text_secondary"],
            "fontSize": "13px"
        })
    ...
    # Analyze PHY layer
    rx_vals = recent['rx_mbps'].dropna()
    rx_mean = rx_vals.mean() if len(rx_vals) > 0 else None
    rx_std = rx_vals.std() if len(rx_vals) > 0 else None
    phy_variance = (rx_std / rx_mean * 100) if rx_mean and rx_mean > 0 else None
    ...
    # Generate insight
    if rtt_mean and rtt_mean > 100 and phy_variance and phy_variance < 10:
        insight = "RTT increased despite stable PHY → congestion likely at network layer"
    elif rtt_mean and rtt_mean < 50 and phy_variance and phy_variance > 30:
        insight = "PHY rate variable but RTT stable → good network layer handling"
    elif rtt_std and rtt_std > 20 and phy_variance and phy_variance < 15:
        insight = "High RTT variance with stable PHY → network layer instability"
    else:
        insight = "Normal operation: PHY and Network layers aligned"
...
def create_chart8_rssi_rtt_scatter(df: pd.DataFrame) -> dict:
    ...
    return {
        "data": scatter_data,
        "layout": {
            **create_base_layout("RSSI vs RTT (Signal Strength vs Latency)"),
            "xaxis": {
                **create_base_layout("")["xaxis"],
                "title": "RSSI (dBm)",
                "tickformat": ""
